module games::range {
    ...
    public entry fun play<T, LLP> (
        ...
        assert!(check_pool_amount(v,math::divide_and_round_up(
            (amount * math::divide_and_round_up(100 * 10000, number)),
            10000
        ) - amount),EGameAmountTooLager);
    ...
    fun test_play_range_win() {
        ...
        play(&mut vault, &mut game_ctl, all, 10000000, 60, &clock,&mut ctx);
        ...
        assert!(pool_amount(&vault) == 989999993333000, 10002);
    ...
    fun test_play_range_lost() {
        ...
        let all = vector::empty<Coin<SUI>>();
        vector::push_back(&mut all, s);
        play(&mut vault, &mut game_ctl, all, 10000000, 40, &clock,&mut ctx);
        let sig = x"8d61d9100567de44682506aea1a7a6fa6e5491cd27a0a0ed349ef6910ac5ac20ff7bc3e09d7c046566c9f7f3c6f3b10104990e7cb424998203d8f7de586fb7fa5f60045417a432684f85093b06ca91c769f0e7ca19268375e659c2a2352b4655";
        let prev_sig = x"176f93498eac9ca337150b46d21dd58673ea4e3581185f869672e59fa4cb390a";
    ...
    fun test_play_range_max_amount() {
        let ctx = tx_context::dummy();
        let sender = tx_context::sender(&mut ctx);
        let scenario = test_scenario::begin(sender);
        ...
        let s = coin::mint_for_testing<SUI>(102000*1000000000, &mut ctx);
        let all = vector::empty<Coin<SUI>>();
        vector::push_back(&mut all, s);
        //1000000 / 40 = 25000 * 10000 / 1000000000 = 250
        // 1000000 / 80 = 12500 * 10000000000000 / 10000 = 125

        play(&mut vault, &mut game_ctl, all, 100000*1000000000, 80, &clock,&mut ctx);
        let sig = x"8d61d9100567de44682506aea1a7a6fa6e5491cd27a0a0ed349ef6910ac5ac20ff7bc3e09d7c046566c9f7f3c6f3b10104990e7cb424998203d8f7de586fb7fa5f60045417a432684f85093b06ca91c769f0e7ca19268375e659c2a2352b4655";
        let prev_sig = x"176f93498eac9ca337150b46d21dd58673ea4e3581185f869672e59fa4cb390a";
