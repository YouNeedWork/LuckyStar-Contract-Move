module newnft::lucky {
    ...
    struct MintState has key,store{
        id:UID,
        supply:u64,
        open:bool,
        minted:Bag,
    ...
    struct LevelCap has key,store{
        id:UID,
    }

    const E_PUBLIC_MINT_CLOSED:u64 = 1001;
    const E_ALREAD_MINTED:u64 = 1002;


    const SHOW_NAME: vector<u8> = b"LuckyStar";
    ...
    fun init(otw:LUCKY,ctx:&mut TxContext){
        ...
        royalty_rule::add<LuckyNFT>(&mut policy,&cap,500,1000);
        ...
        public_transfer(LevelCap{id:object::new(ctx)},tx_context::sender(ctx));
        public_share_object(MintState{id:object::new(ctx),supply:0,open:true,minted:bag::new(ctx)});
    ...
    #[test_only]
    public fun test_cap(ctx:&mut TxContext):LevelCap{
        LevelCap{id:object::new(ctx)}
    }
    ...
    public entry fun public_mint_switch(m:&mut MintState,_cap:&MintCap) {
        m.open = !m.open;
    }

    public entry fun unlimit_mint(m:&mut MintState,ctx:&mut TxContext) {
        assert!(m.open,E_PUBLIC_MINT_CLOSED);
        assert!(!bag::contains(&m.minted,tx_context::sender(ctx)),E_ALREAD_MINTED);
        m.supply = m.supply + 1;
        bag::add(&mut m.minted,tx_context::sender(ctx),1);
        public_transfer(inner_mint(ctx),tx_context::sender(ctx));
    }

    fun inner_mint(ctx:&mut TxContext):LuckyNFT {
        LuckyNFT{
            id:object::new(ctx),
            name:string::utf8(SHOW_NAME),
            img_url:string::utf8(BASE_URI_I),
    ...
    public fun point_up(_:&LevelCap,nft:&mut LuckyNFT,point:u64,_ctx:&mut TxContext){
    ...
    public fun level(nft:&LuckyNFT):u64 {
        nft.level
    }

    public fun point(nft:&LuckyNFT):u64 {
        nft.point
    }
    ...
    fun test_level_and_point(){
        ...
        next_tx(&mut scenario,sender);
        let nft = test_scenario::take_from_sender<LuckyNFT>(&mut scenario);
        let cap = test_scenario::take_from_sender<LevelCap>(&mut scenario);
        assert!(level(&nft)==1,1);
        assert!(point(&nft)==0,1);
        point_up(&mut cap,&mut nft,LUCKYSTAR_NFT_POINT_II,&mut ctx);
        assert!(level(&nft)==LUCKYSTAR_NFT_II,1);
        assert!(point(&nft)==LUCKYSTAR_NFT_POINT_II,1);
        point_up(&mut cap,&mut nft,LUCKYSTAR_NFT_POINT_III,&mut ctx);
        assert!(level(&nft)==LUCKYSTAR_NFT_III,1);
        assert!(point(&nft)==LUCKYSTAR_NFT_POINT_II+LUCKYSTAR_NFT_POINT_III,1);
        point_up(&mut cap,&mut nft,LUCKYSTAR_NFT_POINT_IV,&mut ctx);
        ...
        point_up(&mut cap,&mut nft,LUCKYSTAR_NFT_POINT_V,&mut ctx);
